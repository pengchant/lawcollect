...
<title>添加法律条文详情</title>
<%@include file="../common/func_common/func_header_title.jsp" %>
<div class="panel panel-primary" id="add_laws_div">
    <div class="panel-heading" id="headingTwo">
        <h4 class="panel-title">
            <a href="#collapseTwo" data-toggle="collapse" data-parent="#accordionPanels">录入信息安全法律条文</a>
        </h4>
    </div>
    <div id="collapseTwo" class="panel-collapse collapse in">
        <div class="panel-body">
            <form class="form-horizontal" id="newlawfm">
                <%-- 隐藏域 --%>
                <%-- 法律编号 --%>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-1 col-sm-10 text-center">
                        <button type="button" class="btn btn-info btn-lg btnaddlaw"><i class="icon icon-book"></i>&nbsp;录入
                        </button>&nbsp;&nbsp;&nbsp;&nbsp;
                        <button type="button" class="btn btn-warning btn-lg btntempaddlaw"><i
                                class="icon icon-save"></i>&nbsp;暂存
                        </button>&nbsp;&nbsp;&nbsp;&nbsp;
                        <button type="button" class="btn btn-danger btn-lg btnrsetfm"><i
                                class="icon icon-undo"></i>&nbsp;重置表单
                        </button>&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    $(function () {
        ...
        // 添加法律条文（需要验证字段）
        $("button.btnaddlaw").click(function () {
            var bootstrapValidator = $("#newlawfm").data('bootstrapValidator');
            bootstrapValidator.validate();
            if (bootstrapValidator.isValid()) {
                swal({
                    title: "你确定要录入吗？",
                    text: "录入后的内容将无法修改，系统将提交给其他人员校验！",
                    icon: "warning",
                    buttons: {
                        cancel: "取消",
                        sure: "确定",
                    ...
                    if (value) {
                        // 下面发起录入请求
                        dopost("/law/tempsave/1", $("#newlawfm").serializeObject()).then(function (v) {
                            if (v.status == 200) {
                                swal("", "录入成功!", "success", {
                                    button: false,
                                    timer: 1000
                                })
                                $("button.btnrsetfm").click();
                            } else {
                                swal("", v.msg, "error", {
                                    button: false,
                                    timer: 1000
        ...
        $("button.btntempaddlaw").click(function () {
            if ($("#lawname").val() == "" || $("#version").val() == "") {
                swal("", "请您至少输入法律名称和版本，否则系统无法为你保存!", "error", {
                    button: false,
                    timer: 3000
                })
            } else {
                swal({
                    ...
                    if (value) {
                        // 下面发起录入请求
                        dopost("/law/tempsave/0", $("#newlawfm").serializeObject()).then(function (v) {
                            if (v.status == 200) {
                                swal("", "暂存成功！你可以继续修改！", "success", {
                                    button: false,
                                    timer: 1000
                                })
                            } else {
        ...
        // 重置表单
        $("button.btnrsetfm").click(function () {
            $("#lawname").focus();
            $('#newlawfm').data('bootstrapValidator').resetForm(true);
        });
        // 返回列表
        $("button.btnbackaddlist").click(function () {
            window.location.href = "/law/stuadd";
        });
